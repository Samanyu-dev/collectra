...
export default async function ArtistsPage(
  ...
  const [artistsRaw, total] = await Promise.all([
  ...
  const previewImages = await getImagesForEntities("Card", artistsRaw.flatMap((a) => a.cards.map((c) => c.id)));
  const artists = artistsRaw.map((a) => ({
    ...a,
    previewImage: a.cards[0] ? previewImages.get(a.cards[0].id)?.[0]?.url ?? null : null,
  }));
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-8 pb-32 space-y-8">
      <div>
        <p className="flex items-center gap-2 text-foreground/50 text-sm font-mono uppercase tracking-widest mb-2">
          <Brush size={16} /> Artists
        </p>
        <h1 className="text-3xl md:text-4xl font-display font-bold tracking-tight">
          Every illustrator in the graph
        </h1>
        <p className="text-foreground/50 mt-2 max-w-2xl">{total.toLocaleString()} artists tracked, ranked by cards drawn.</p>
      </div>
      ...
      {artists.length === 0 ? (
        <div className="text-center py-24 text-foreground/40">No artists found.</div>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-4 py-8">
          {page > 1 && (
            <Link href={`/artists?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ''}`} className="px-5 py-2.5 rounded-full bg-foreground/5 hover:bg-foreground/10 text-sm transition-colors">
              Previous
            </Link>
          )}
