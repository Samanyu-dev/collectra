...
**Correcting this file's own prior claim, 2026-08-04**: Tier 1 is no longer deferred — a real eBay Browse API integration (Production credentials, client-credentials OAuth) has been built, verified against live data, and is running on a daily cron. It observes active listings (asks), not sold comps — eBay's Marketplace Insights API (the actual sold-comp source) is a separate, more tightly-gated product this integration doesn't have access to. See §19.
...
**Update 2026-07-23: deployed and verified in production**, with one real finding along the way. Commit `3d74ee7`, tag `v0.5.0-beta`, `CRON_SECRET` set on Vercel Production, deployed to `https://collectra-plum.vercel.app`. Verified live: auth flow (real signup/login against a synthetic test account, cleaned up afterward), ownership isolation, cron 401-without-secret and real-execution-with-one, the admin dashboard's role gate and rendering, and — the actual point of this whole exercise — the production database changing during a real sync run (1,721 → 11,123+ `PriceObservation` rows).

**What that live run found**: a single cron invocation cannot finish the ~174-set catalog inside Vercel's function duration limit. The run got through 14 sets before progress stopped. This is a scheduling/architecture gap, not a correctness bug — the pipeline itself (auth, writes, aggregation, confidence, outlier exclusion) was correct throughout. See §18 for the fix.

### 18. Phase 5.1 — incremental/resumable catalog sync

**Decided and implemented** in the same sitting the gap was found, rather than left as a known issue. The fix isn't "make it faster" (already resisted — see §"What actually shipped" below on not chasing further throughput after the 1.9x batching win) — it's making each invocation do a *bounded* amount of work and remember where it stopped.

- **`DataSource.syncCursor: String?`** — the id of the last set that fully completed, not an array index. Index-based resume would silently skip or reprocess sets if the upstream API ever reorders its list; id-based resume is stable regardless.
- **`resolveStartIndex(items, cursor)`** (`src/lib/pricing/resume-cursor.ts`) — pure function, no I/O: null/missing cursor → start at 0; cursor found → resume right after it; cursor was the last item → wrap to 0 (a full lap just finished). Tested directly, including the reorder case.
- **Time-boxed, not count-boxed**: `syncPokemonPrices` takes a `timeBudgetMs` (default 240s, under the route's 300s `maxDuration`, leaving buffer for one slow in-flight set plus the response) and stops *before* starting a new set once the budget's spent — never mid-set.
- **Cursor persists after every completed set**, not just at the end of the run — so a hard kill mid-invocation (the exact scenario that motivated this) loses at most the one set in flight, not the whole run's progress.
- **Idempotent by construction, not by dedup logic**: if a set gets interrupted after some writes but before its cursor update, the next invocation reprocesses that same set from scratch. `PriceObservation` is append-only, so the result is a few duplicate observations (mild, harmless corroboration), never a wrong price — a deliberate simplicity trade-off over building exactly-once semantics for a case that barely matters.
- **`limitSets` (the existing local/CLI test parameter) is unaffected** — deliberately bypasses the cursor entirely, so local verification runs stay a fixed, reproducible prefix.
- **Verified with a real three-run integration test** against the live database, not simulated: run 1 (budget too tight, 0 sets — proves the guard doesn't false-positive into corrupting state), run 2 (processed Base, cursor → `base1`), run 3 (correctly resumed at index 1, processed Jungle, cursor → `base2` — proving it advanced rather than restarting).
- **Admin page** now shows each source's resume cursor, so "how far through the catalog are we" is visible without a DB query.

**A real bug found by cross-checking two live production runs against each other, fixed before it mattered much.** The first cut set `syncCursor` to "whichever set most recently succeeded." Deployed, then verified with two real cron invocations against the live app — run 1 hit a real transient failure (`Dragon`/`ex3`, a genuine upstream 500) followed by several more sets that succeeded. Because the cursor only tracked "most recent success," those later successes overwrote it past `Dragon` — the failed set wouldn't have been retried again until a full ~155-set lap wrapped back around to it, not "soon" by any reasonable definition. The unit test written alongside the original fix didn't catch this because it hand-set the follow-up run's starting cursor to a value that didn't match what the real code actually produces.

**Fixed to a real watermark**: the cursor now only advances through a *contiguous* run of successes and freezes the instant a set fails, even if later sets in the same run succeed. The corrected test drives the real function through this exact scenario and asserts the cursor is never set to the set that came after the failure, not just never set to the failed set itself. The one set already skipped by the original bug (`ex3`/Dragon) was manually rewound on the live `DataSource` row so it gets retried promptly rather than waiting for the next full lap — a one-time, surgical correction of the bug's actual (small, contained) blast radius, not a design change.

**Known accepted risk, not solved here**: if one specific set fails on *every* future attempt (not just transiently — nothing observed all session behaves this way; every failure so far has been a passing upstream 500), the watermark would never advance past it, stalling the whole catalog rather than skipping around it. Deliberately not building bounded-retry-then-skip logic for a scenario with zero real precedent — visible today via `failedSets` in the function's logs/response if it ever happens, worth a real fix only if it does.

### 19. Phase 5.2 — eBay Tier 1 (real credentialed API, implemented 2026-08-03/04)

Real Production eBay credentials provisioned (`EBAY_APP_ID`/`EBAY_DEV_ID`/`EBAY_CERT_ID`), confirmed live end-to-end (a raw client-credentials grant against `api.ebay.com/identity/v1/oauth2/token` returned a real token) before any code was written, per this codebase's own "verify against real docs/APIs, don't assume" rule.

**What this observes — read before treating it as sold-comp data**: the Browse API's `item_summary/search` returns **active listings (asks)**, not completed sales. Every row this writes is `PriceObservation.kind: "LISTING"`, never `"SOLD"`. Real sold comps would need eBay's separately-gated Marketplace Insights API, which this integration does not have access to — not attempted here, and downstream code must not relabel these as sold prices.

**Architecture** (`src/ingestion/ebay/`):
- `auth.ts` — cached Application access token (client-credentials grant), 60s expiry safety margin.
- `api-client.ts` — `searchItems()` plus three real anti-contamination filters, each added after a live-data finding, not speculatively:
  - `isLikelyBulkListing()` — keyword + regex denylist for lots/bundles/"you pick"/"complete set" listings, extended after a real spot-check found "full set of 15 CARDS" phrasing the original keyword list missed.
  - `isLikelyGraded()` — excludes PSA/BGS/SGC/CGC slabs from base-card price aggregation (a real spot-check found a $400 graded slab corrupting a $2 raw-card median).
  - `titleMatchesCard()` — hard-rejects a search result unless the card's number appears as a distinct token in the actual returned title AND at least half the card name's meaningful words match, plus a small Chrome sub-line denylist (`sapphire`/`cosmic`/`velocity`/etc., skipped when the card's own set name legitimately contains that word). Added after two real, different wrong-match findings: eBay's relevance-ranked search returning a different card number entirely (`Shikamaru Nara #100` matching a listing titled `NX-159`), and a same-number-same-name-wrong-product match (a `#CA-GM` premium "Chrome Sapphire" parallel matching the mainline catalogued card of the same number).
- `sync-prices.ts` — per-variant CLI pilot tool (median/outlier-drop helpers, reused by the sweep below).
- `sweep-catalog.ts` — the real, resumable, **paginated** full-catalog sweep. Pagination strategy decided after checking the actual bottleneck: eBay's own quota (~4,800/day budgeted) is *not* the limit — Vercel's 300s function `maxDuration` is, at ~1.5–2.5s/card observed latency. Design:
  - A `{tier}:{cardId}` resume cursor on `DataSource.syncCursor` (id-based, same stability rationale as §18's Pokémon cursor).
  - **Tiered priority order**, not a flat catalog walk: tier 0 = the user's own owned collection (real user request — their portfolio dashboard showed $0 with no market prices), then four named franchise tiers (Soccer/Football, F1, Naruto, Marvel — real `Set.name` regex matches confirmed against the live DB, not guessed), then everything else ("REST").
  - A bounded 1,000-row window per invocation that tops up across tier boundaries and wraps back to tier 0 once REST is exhausted — rewritten after a real timed run showed loading the *entire* catalog unbounded on every invocation alone cost ~55–60s.
  - Real rate limiting (`throttleRequest`, reused from Phase 5's multi-window limiter) plus the cursor-freezes-on-a-contiguous-failure guarantee from §18, applied here too and covered by an equivalent test (`src/ingestion/ebay/__tests__/sweep-catalog.test.ts`).
- Images: eBay's own listing photo for the best-matched result is downloaded and **re-hosted in Supabase Storage** (`EBAY_LISTING_PHOTO` media usage), not left as a bare eBay-CDN hotlink — a deliberate departure from every other ingestion source's hotlink-only convention, made because eBay listings expire far faster than official artwork. `attachHotlinkImage()` gained a `replaceExisting` mode for this (a re-run whose match quality improves must replace a stale/wrong image, not accumulate a second attachment row beside it) — the original unscoped-mediaId check let a wrong image attached during a buggy window win display forever via stable-sort tie order; fixed. `pick-primary-image.ts` centralizes the cross-source display-priority order (`OFFICIAL_ARTWORK > THUMBNAIL > EBAY_LISTING_PHOTO > LISTING_PHOTO`) so every page agrees, replacing ~12 pages' worth of inline `.find()` chains that silently never matched the new usage tag.
- `/api/cron/ebay-price-sync` — same `CRON_SECRET`-gated shape as `/api/cron/price-sync`, scheduled daily (`30 6 * * *` in `vercel.json`).

**A real contamination bug found and fixed mid-sweep**: an early version of the query-matching logic let wrong-number and wrong-subline results through (the two findings behind `titleMatchesCard()` above). Fixed forward, then a one-time purge script (`src/scripts/purge-contaminated-ebay-data.ts`) removed the specific pre-fix `PriceObservation` rows (by `sourceId` + `createdAt` before the verified fix-deployment instant, not a blanket delete) and recomputed `CurrentPrice` for every affected variant.

**Verified against real production data, not mocked** (checked live 2026-08-04): 2,493 clean `PriceObservation` rows across 706 variants, 706 real `EBAY_LISTING_PHOTO` images downloaded and re-hosted, 0 pre-fix contaminated rows remaining, sweep cursor actively advancing. 21 new unit tests (`src/ingestion/ebay/__tests__/`) cover the filter/matching logic and the tiered resume-cursor guarantee — mirroring §18's Pokémon test pattern.

**Compliance note, read in full in `sync-prices.ts`'s file header**: eBay's API License Agreement §8.1(b)(1) technically requires deleting stored eBay Content once its source listing is no longer public; no re-verification/deletion job exists for this yet. **User decision (explicit, informed)**: price/image data is kept as a refreshed reference point, not a live listing mirror — an accepted risk, not an oversight, extended to the re-hosted images (a stronger copy of eBay Content than the clause originally contemplated). §8.1(c)'s 6-hour-freshness requirement and §8.1(d.1.D)'s category-average-price restriction are both discussed in the same file header; worth a real legal read before this scales past its current pilot-to-production-cron state.

**Update 2026-08-05 — owned-collection tier-0 completed a full lap, plus a real second matching bug found and fixed.** Per an explicit user request ("finish pricing the cards I own first"), ran `sweep-catalog.ts` locally against real production data until tier 0 fully exhausted (cursor advanced into tier 1) — every one of the user's 514 distinct owned cards has now been visited by the eBay search at least once. Before this run: 308/514 priced. After: still only 309/514 — a live spot-check against real eBay results (not assumed) found why: `numberAppearsInTitle()`'s word-boundary match required the card's own compound insert-number formatting (e.g. our catalog's `"IN 6"`, with a literal space) to appear verbatim in the listing title, but real listings write it without the space (`"#IN6"`) — silently rejecting genuine, correct, non-bulk/non-graded matches. Fixed: internal whitespace in the card number is now optional (`\s*`) in the match regex, verified against the real listings that were being wrongly rejected, with a regression test added. Re-ran a targeted sync against exactly the 205 still-missing owned variants after the fix: only 5 more resolved. The remaining ~200 are confirmed, via direct live spot-checks (not assumed), to be a genuine no-market situation — Team Badge cards (not individually sold), thin-market lesser-known players, and specific limited-edition inserts with zero matching eBay listings at all (only bulk/pick-your-card results, correctly rejected). This is an honest ceiling of eBay's active-listing coverage for this specific collection, not a remaining bug.

**Deliberately not built**: a listing-liveness recheck/delete job (the compliance gap above), telemetry (throughput, contamination rate over time), and running the sweep's `--unbounded` CLI mode as a long-lived background process (the only way to lap the full ~32,107-card catalog in the ~6–7 days simple division suggests, rather than the months a daily 300s-capped cron alone would take).
